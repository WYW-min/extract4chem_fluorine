...
def enhance_config(raw_config):
    ...
    if len(str_array) >= 2:
        ...
        config["OUTPATH"] = (
            config["OUTPATH"] + f"/{config["name"]}_{checkpoint_suffix}_{now_str}.json"
        )
    else:
        config["OUTPATH"] = config["OUTPATH"] + f"/{config["name"]}_{now_str}.json"
    ...
    for record in orjsonl.stream(config["CHECKPOINT_PATH"]):
        ...
        if record.get(result_key, {}).get("error") is None:
...
APP_CONFIG = {
    ...
    "CHECKPOINT_PATH": "./data/out/分子筛/20260108160000/app.jsonl",
}
...
CHAR_CONFIG = {
    ...
    "LLM": "gpt_low",
    "INPATH": "./data/out/分子筛/20260108160000/main.jsonl",
    "OUTPATH": f"./data/out/分子筛/char.jsonl",
    "PROMPT_PATH": "./prompts/characterization.txt",
    "READ_NUM": 300,
    "SCHEMA_DEFINE": CharacterizationResult,
    "CHECKPOINT_PATH": "data/out/分子筛/20260108160000/char.jsonl",
}

if __name__ == "__main__":
    configs = [APP_CONFIG, CHAR_CONFIG]
