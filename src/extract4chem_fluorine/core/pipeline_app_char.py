...
import orjson
from extract4chem_fluorine.entities.application import ApplicationResult
from extract4chem_fluorine.entities.characterization import CharacterizationResult
from xopen import xopen
from tqdm import tqdm
from extract4chem_fluorine.tools.data_tool import count_jsonlines, get_section
...
def get_final_data(record:dict, chain_response:BaseModel|Exception, other_info:None | dict = None)->dict:
    if isinstance(chain_response, Exception):
        chain_response = {"error": str(chain_response)}

    final_data = (other_info or {}) \
            | record \
            | {"syn_extract_result" : chain_response} 
    
    if isinstance(final_data["syn_extract_result"], BaseModel):
        final_data["syn_extract_result"] = final_data["syn_extract_result"].model_dump(mode="json")
    return final_data
    ...
    for record in orjsonl.stream(inpath):
        ...
        for material_info in record["main_extract_result"]["parse"]["material_ids"]:
            ...
            yield {
                ...
                "chain_input" : {
                    "paper_info": input_data["specific_section"],
                    "material_info": {k:v  for k,v in material_info.items() if k in need_cols}    
                }
            }
...
APP_CONFIG = {
    "name" : "app",
    "LLM" : "gpt",
    "INPATH" : "./data/out/main_20251109.json",
    "OUTPATH" : f"./data/out",
    "PROMPT_PATH" : "./prompts/application.txt",
    "READ_NUM" : 20,
    "SCHEMA_DEFINE" : ApplicationResult,
    "CHECKPOINT_PATH" : "./data/out/app_20251111144515_20251111194455.json"
}
...
CHAR_CONFIG = {
    ...
    "INPATH" : "./data/out/main_20251109.json",
    "OUTPATH" : f"./data/out",
    "PROMPT_PATH" : "./prompts/characterization.txt",
    "READ_NUM" : 40,
    "SCHEMA_DEFINE" : CharacterizationResult,
    "CHECKPOINT_PATH" : "./data/out/char_20251111173609_20251112093545.json"
}
...
def enhance_config(raw_config):
    ...
    checkpoint_suffix = Path(config["CHECKPOINT_PATH"]).stem.split("_", maxsplit = 1)[1]
    config["OUTPATH"] = config["OUTPATH"] + f"/{config["name"]}_{checkpoint_suffix}_{now_str}.json"
...
async def main(config=APP_CONFIG):
    config = enhance_config(config)
    
    main_chain = get_chain(Path(config["PROMPT_PATH"]), config["SCHEMA_DEFINE"], config["LLM"])
    inpath, outpath = Path(config["INPATH"]), Path(config["OUTPATH"])
    pbars = tqdm(desc = "抽取进度", total = count_jsonlines(inpath))
    with xopen(outpath, "wb") as fout:
        for records in batched(yield_input(inpath), n = config["READ_NUM"]):
            ...
            final_data = [get_final_data(r["id"], run_chain_result) for r, run_chain_result in zip(records, ga_coroutine_results)]
            fout.write(b"".join(orjson.dumps(d) + b"\n" for d in final_data ))
            pbars.update(len(records))

            
if __name__ == "__main__":
    configs = [APP_CONFIG, CHAR_CONFIG] # APP_CONFIG, 
