
from pathlib import Path
from langchain_text_splitters import MarkdownHeaderTextSplitter
from bidict import bidict
from typing import Dict, List
import orjson
from .global_state_manager import gs_manager
def add_nowstr(p:Path):
    new_p = p.parent / gs_manager["timestamp"] / p.name
    new_p.parent.mkdir(exist_ok = True, parents = True)
    return new_p


def count_jsonlines(filepath):
    with open(filepath, 'rb') as f:
        return sum(1 for _ in f)

def get_section(splited_texts: List[Dict[str,str]]) -> str:
    exp_infos, specific_infos, result_infos, index = [], [], [], 0
    while index<=len(splited_texts)-1:
        
        if "REFERENCE" not in splited_texts[index]["content_title"].upper():
            text = f"# {splited_texts[index]["content_title"]}\n{splited_texts[index]["content"]}"
            exp_infos.append(text)
            if index != 0 and \
                "INTRODUCTION" not in splited_texts[index]["content_title"].upper() and \
                "ABSTRACT" not in splited_texts[index]["content_title"].upper():
                specific_infos.append(text)
                
        if any(f in splited_texts[index]["content_title"].upper() for f in ["RESULTS", "DISCUSSION", "CONCLUSION"]):
            result_infos.append(f"# {splited_texts[index]['content_title']}\n{splited_texts[index]['content']}")
            for block in splited_texts[index+1:]:        
                if any(f in block["content_title"].lower() for f in ["conclusion", "acknowledgement", "reference"]):
                    break
                result_infos.append(f"# {block['content_title']}\n{block['content']}")
        
        index += 1
    input_data = {
        "effective_section" : "\n\n".join(exp_infos),
        "result_and_discussion" : "\n\n".join(result_infos),
        "specific_section" : "\n\n".join(specific_infos)
    }    
    return input_data


def processDoc(markdown_document)->List[Dict[str,str]]:
    """
    处理文档段落列表 (md_splits)，提取相关内容，清理标题，替换 Markdown 媒体，
    并过滤掉不相关的段落。

    Args:
        markdown_document (str): Markdown 文档内容。

    Returns:
        list: 包含已处理和过滤的文档段落信息的列表。
              每个元素包含 'title_level'、'content_title' 和 'content' 键。
    """
    headers_to_split_on = [
        ("#", "H1"),
        ("##", "H2"),
        ("###", "H3"),
        ("####", "H4")
    ]
    # 创建 Markdown 分割器实例
    splitter = MarkdownHeaderTextSplitter(
        headers_to_split_on, 
        strip_headers=False
    )
    levels = [x[1] for x in headers_to_split_on]
    # 分割 Markdown 文档
    md_splits = splitter.split_text(markdown_document)
    bimap_level = bidict(headers_to_split_on)
    processed_splits = []
    # 遍历分割后的文档段落并处理
    for record in md_splits:
        if not record.metadata:
            continue

        split_info = {}
        cur_level = None
        cur_title = None

        # 查找有效的标题级别和标题文本
        for k, v in record.metadata.items():
            if k in levels:
                cur_level, cur_title = k, v
                split_info["title_level"] = k
                split_info["content_title"] = cur_title.strip()
                break
        
        # 如果没有找到有效级别则跳过
        if cur_level is None:
            continue

        # 构造标题字符串并从内容中移除
        title_str = " ".join([
            bimap_level.inverse.get(cur_level, ""),
            cur_title
        ])
        split_info["content"] = record.page_content.strip().lstrip(title_str).strip()
        processed_splits.append(split_info)
    return processed_splits
    
    

import re
from typing import Dict, List, Optional, Tuple


def extract_doi(text: str) -> Optional[str]:
    """
    从论文文本中提取DOI
    
    参数:
        text: 论文文本内容
        
    返回:
        str: 提取到的DOI,如果未找到则返回None
        
    示例:
        >>> extract_doi("DOI: 10.1038/s41586-020-2649-2")
        '10.1038/s41586-020-2649-2'
        >>> extract_doi("https://doi.org/10.1016/j.cell.2021.01.001")
        '10.1016/j.cell.2021.01.001'
        >>> extract_doi("没有DOI的文本")
        None
    """
    if not text:
        return None
    
    # DOI的正则表达式模式
    # DOI通常以10.开头,后面跟着数字和斜杠,以及各种字符
    patterns = [
        # 匹配 DOI: 10.xxxx/xxx 格式
        r'\bdoi:\s*(?:https?://(?:dx\.)?doi\.org/)?(10\.\d{4,}/[^\s]+)',
        # 匹配 https://doi.org/10.xxxx/xxx 格式
        r'https?://(?:dx\.)?doi\.org/(10\.\d{4,}/[^\s]+)',
        # 匹配 dx.doi.org/10.xxxx/xxx 格式
        r'dx\.doi\.org/(10\.\d{4,}/[^\s]+)',
        # 匹配纯DOI格式 10.xxxx/xxx (更严格的匹配)
        r'\b(10\.\d{4,}/[-._;()/:a-zA-Z0-9]+)',
    ]
    
    for pattern in patterns:
        match = re.search(pattern, text, re.IGNORECASE)
        if match:
            doi = match.group(1)
            # 清理DOI末尾可能的标点符号
            doi = re.sub(r'[.,;:)\]}\s]+$', '', doi)
            return doi
    
    return None
