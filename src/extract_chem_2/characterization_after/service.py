...
from .helpers import build_output_record, is_good_predict_record, merge_parse_records
from .models import CharacterizationAfterArgs, CharacterizationAfterResult, CharacterizationAfterStats
from .storage import iter_predict_records, load_jsonl, peek_first_record, resolve_output_paths, write_manifest


def run_characterization_after(args: CharacterizationAfterArgs) -> CharacterizationAfterResult:
    ...
    if not isinstance(run_id, str) or not run_id:
        raise ValueError(f"Missing run_id in characterization_before record: {args.before_jsonl}")
    ...
    for record in iter_predict_records(paths.predict_temp_dir, args.encoding):
        ...
        if not isinstance(task_id, str) or not task_id.strip():
            continue
        if not is_good_predict_record(record):
            continue
    ...
    with paths.jsonl_path.open("w", encoding="utf-8") as fout:
        pbar = tqdm(total=len(tasks), desc="characterization_after", unit="task")
        for task in tasks:
    ...
    return CharacterizationAfterResult(run_id=run_id, paths=paths, stats=stats)


def print_run_summary(result: CharacterizationAfterResult) -> None:
    print(f"run_id: {result.run_id}")
    print(
        "characterization_after done: "
...
def run_main(args: CharacterizationAfterArgs) -> CharacterizationAfterResult:
    return run_characterization_after(args)
