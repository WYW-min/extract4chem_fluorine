...
from .builder import CONFIG_PATH, build_characterization_task
from .models import (
    CharacterizationBeforeArgs,
    CharacterizationBeforeResult,
    CharacterizationBeforeStats,
)
from .storage import (
...
def run_characterization_before(args: CharacterizationBeforeArgs) -> CharacterizationBeforeResult:
    ...
    no_result_match_count = 0
    result_fallback_count = 0
        ...
        for record in after_records:
            ...
            for polymer_index, polymer in enumerate(polymers, start=1):
                if not isinstance(polymer, dict):
                    continue
                task = build_characterization_task(
                    ...
                    sections=sections,
                    result_window_lines=args.result_window_lines,
                )
                route_stats = task["route_stats"]
                if route_stats["method_window_count"] == 0:
                    no_method_match_count += 1
                if route_stats["result_window_count"] == 0:
                    no_result_match_count += 1
                if route_stats["used_result_fallback"]:
                    result_fallback_count += 1
                if route_stats["method_window_count"] == 0 and route_stats["result_window_count"] == 0:
    ...
    stats = CharacterizationBeforeStats(
        ...
        no_result_match_count=no_result_match_count,
        result_fallback_count=result_fallback_count,
    )
    write_manifest(
        paths=paths,
        run_id=run_id,
    ...
    return CharacterizationBeforeResult(run_id=run_id, paths=paths, stats=stats)


def print_run_summary(result: CharacterizationBeforeResult) -> None:
    print(f"run_id: {result.run_id}")
    print(
        "characterization_before done: "
    ...
    print(f"no result match count: {result.stats.no_result_match_count}")
    print(f"result fallback count: {result.stats.result_fallback_count}")
