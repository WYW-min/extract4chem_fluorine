...
from .models import CharacterizationBeforePaths, CharacterizationBeforeStats
...
def resolve_output_paths(
    *,
    ...
    out_jsonl: str | None,
) -> CharacterizationBeforePaths:
    if out_jsonl:
        jsonl_path = Path(out_jsonl)
        stage_dir = jsonl_path.parent
    else:
        if inpath.parent.name == "main_signal_after":
            stage_dir = inpath.parent.parent / "characterization_before"
        else:
            stage_dir = inpath.parent / "characterization_before"
    ...
    return CharacterizationBeforePaths(
...
def write_manifest(
    *,
    ...
    manifest = {
        ...
        "no_result_match_count": stats.no_result_match_count,
        "result_fallback_count": stats.result_fallback_count,
