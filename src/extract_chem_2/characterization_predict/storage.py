...
from .models import CharacterizationPredictPaths, CharacterizationPredictStats
...
def resolve_output_paths(*, inpath: Path, out_jsonl: str | None) -> CharacterizationPredictPaths:
    if out_jsonl:
        ...
        if inpath.parent.name == "characterization_before":
            stage_dir = inpath.parent.parent / "characterization_predict"
        else:
            stage_dir = inpath.parent / "characterization_predict"
        temp_dir = stage_dir / "temp"
        jsonl_path = temp_dir / f"{make_now_id()}.jsonl"

    temp_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / "manifest.json"
    return CharacterizationPredictPaths(
        stage_dir=stage_dir,
        temp_dir=temp_dir,
...
def write_manifest(
    *,
    ...
    manifest = {
        "stage": "characterization_predict",
        "run_id": run_id,
