...
from .models import MainSignalAfterPaths, MainSignalAfterStats
...
) -> MainSignalAfterPaths:
    if output_jsonl:
        jsonl_path = Path(output_jsonl)
        stage_dir = jsonl_path.parent
    else:
        if before_jsonl.parent.name == 'main_signal_before':
            stage_dir = before_jsonl.parent.parent / 'main_signal_after'
        else:
            stage_dir = before_jsonl.parent / 'main_signal_after'
        jsonl_path = stage_dir / before_jsonl.name

    if predict_temp_dir is not None:
        temp_dir = predict_temp_dir
    elif before_jsonl.parent.name == 'main_signal_before':
        temp_dir = before_jsonl.parent.parent / 'main_signal_predict' / 'temp'
    else:
        temp_dir = before_jsonl.parent / 'main_signal_predict' / 'temp'

    stage_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / 'manifest.json'
    return MainSignalAfterPaths(
        stage_dir=stage_dir,
...
def write_manifest(
    *,
    ...
    manifest = {
        ...
        'good_predict_count': stats.good_predict_count,
