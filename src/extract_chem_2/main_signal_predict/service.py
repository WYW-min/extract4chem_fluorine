...
from src.extract_chem_2.entities.main_signal import MainSignalResult
...
from .models import MainSignalPredictArgs, MainSignalPredictResult, MainSignalPredictStats
from .storage import (
...
async def run_main_signal_predict(args: MainSignalPredictArgs) -> MainSignalPredictResult:
    ...
    if not isinstance(run_id, str) or not run_id:
        raise ValueError(f'Missing run_id in main_signal_before record: {args.inpath}')
    ...
    with paths.jsonl_path.open('w', encoding='utf-8') as fout:
        pbar = tqdm(total=len(pending_tasks), desc='main_signal_predict', unit='task')
    ...
    return MainSignalPredictResult(run_id=run_id, paths=paths, stats=stats)


def print_run_summary(result: MainSignalPredictResult) -> None:
    print(f'run_id: {result.run_id}')
    print(
        'main_signal_predict done: '
...
def main_async(args: MainSignalPredictArgs) -> MainSignalPredictResult:
    return asyncio.run(run_main_signal_predict(args))
