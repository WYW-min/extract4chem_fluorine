...
from .models import MainSignalPredictPaths, MainSignalPredictStats
...
def resolve_output_paths(
    *,
    ...
    if out_jsonl:
        ...
        if inpath.parent.name == 'main_signal_before':
            stage_dir = inpath.parent.parent / 'main_signal_predict'
        else:
            stage_dir = inpath.parent / 'main_signal_predict'
        temp_dir = stage_dir / 'temp'
        jsonl_path = temp_dir / f'{make_now_id()}.jsonl'

    temp_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / 'manifest.json'
    return MainSignalPredictPaths(
        stage_dir=stage_dir,
        temp_dir=temp_dir,
