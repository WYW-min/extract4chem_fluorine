...
from .models import ProcessAfterPaths, ProcessAfterStats
...
def resolve_output_paths(
    *,
    ...
    if output_jsonl:
        ...
        if before_jsonl.parent.name == "process_before":
            stage_dir = before_jsonl.parent.parent / "process_after"
        else:
            stage_dir = before_jsonl.parent / "process_after"
        jsonl_path = stage_dir / before_jsonl.name

    if predict_temp_dir is not None:
        temp_dir = predict_temp_dir
    elif before_jsonl.parent.name == "process_before":
        temp_dir = before_jsonl.parent.parent / "process_predict" / "temp"
    else:
        temp_dir = before_jsonl.parent / "process_predict" / "temp"

    stage_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / "manifest.json"
    return ProcessAfterPaths(
        stage_dir=stage_dir,
