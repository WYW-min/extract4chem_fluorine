...
from .models import ProcessPredictArgs, ProcessPredictResult, ProcessPredictStats
from .storage import collect_done_task_ids, load_tasks, peek_first_record, resolve_output_paths, write_manifest


async def run_process_predict(args: ProcessPredictArgs) -> ProcessPredictResult:
    ...
    if not isinstance(run_id, str) or not run_id:
        raise ValueError(f"Missing run_id in process_before record: {args.inpath}")
    ...
    with paths.jsonl_path.open("w", encoding="utf-8") as fout:
        pbar = tqdm(total=len(pending_tasks), desc="process_predict", unit="task")
    ...
    return ProcessPredictResult(run_id=run_id, paths=paths, stats=stats)


def print_run_summary(result: ProcessPredictResult) -> None:
    print(f"run_id: {result.run_id}")
    print(
        "process_predict done: "
...
def main_async(args: ProcessPredictArgs) -> ProcessPredictResult:
    return asyncio.run(run_process_predict(args))
