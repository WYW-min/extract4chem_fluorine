...
from .models import ProcessPredictPaths, ProcessPredictStats
...
def resolve_output_paths(*, inpath: Path, out_jsonl: str | None) -> ProcessPredictPaths:
    if out_jsonl:
        ...
        if inpath.parent.name == "process_before":
            stage_dir = inpath.parent.parent / "process_predict"
        else:
            stage_dir = inpath.parent / "process_predict"
        temp_dir = stage_dir / "temp"
        jsonl_path = temp_dir / f"{make_now_id()}.jsonl"

    temp_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / "manifest.json"
    return ProcessPredictPaths(
        stage_dir=stage_dir,
        temp_dir=temp_dir,
