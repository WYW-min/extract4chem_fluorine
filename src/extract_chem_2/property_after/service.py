...
from .models import PropertyAfterArgs, PropertyAfterResult, PropertyAfterStats
from .storage import iter_predict_records, load_jsonl, peek_first_record, resolve_output_paths, write_manifest


def run_property_after(args: PropertyAfterArgs) -> PropertyAfterResult:
    ...
    if not isinstance(run_id, str) or not run_id:
        raise ValueError(f"Missing run_id in property_before record: {args.before_jsonl}")
    ...
    with paths.jsonl_path.open("w", encoding="utf-8") as fout:
        pbar = tqdm(total=len(tasks), desc="property_after", unit="task")
        for task in tasks:
    ...
    return PropertyAfterResult(run_id=run_id, paths=paths, stats=stats)


def print_run_summary(result: PropertyAfterResult) -> None:
...
def run_main(args: PropertyAfterArgs) -> PropertyAfterResult:
    return run_property_after(args)
