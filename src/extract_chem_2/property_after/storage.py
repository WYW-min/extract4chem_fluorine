...
from .models import PropertyAfterPaths, PropertyAfterStats
...
def resolve_output_paths(
    *,
    ...
    if output_jsonl:
        ...
        if before_jsonl.parent.name == "property_before":
            stage_dir = before_jsonl.parent.parent / "property_after"
        else:
            stage_dir = before_jsonl.parent / "property_after"
        jsonl_path = stage_dir / before_jsonl.name

    if predict_temp_dir is not None:
        temp_dir = predict_temp_dir
    elif before_jsonl.parent.name == "property_before":
        temp_dir = before_jsonl.parent.parent / "property_predict" / "temp"
    else:
        temp_dir = before_jsonl.parent / "property_predict" / "temp"

    stage_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = stage_dir / "manifest.json"
    return PropertyAfterPaths(
        stage_dir=stage_dir,
