...
from .models import PropertyPredictArgs, PropertyPredictResult, PropertyPredictStats
from .storage import collect_done_task_ids, load_tasks, peek_first_record, resolve_output_paths, write_manifest


async def run_property_predict(args: PropertyPredictArgs) -> PropertyPredictResult:
    ...
    if not isinstance(run_id, str) or not run_id:
        raise ValueError(f"Missing run_id in property_before record: {args.inpath}")
    ...
    with paths.jsonl_path.open("w", encoding="utf-8") as fout:
        pbar = tqdm(total=len(pending_tasks), desc="property_predict", unit="task")
    ...
    return PropertyPredictResult(run_id=run_id, paths=paths, stats=stats)


def print_run_summary(result: PropertyPredictResult) -> None:
    print(f"run_id: {result.run_id}")
    print(
        "property_predict done: "
...
def main_async(args: PropertyPredictArgs) -> PropertyPredictResult:
    return asyncio.run(run_property_predict(args))
